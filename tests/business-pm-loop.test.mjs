...
async function testValCore011_UnsafeProposalsBlockedWithEscalationRecords() {
  ...
  const result = await callTool("business_pm_loop", {
    ...
    correlation_id: correlationId,
    proposed_actions: [
      { action: "full_vps_restart", type: "restart full VPS", description: "Full VPS restart requiring approval" },
      { action: "create_vps_snapshot", type: "snapshot", description: "Create VPS snapshot" },
      { action: "stop_production_project", type: "stop project nginx-proxy", description: "Stop production nginx proxy" },
    ],
  });
  ...
  const approvalRequests = result.propose?.approval_requests || [];
  assert(approvalRequests.length > 0, `Should have at least one approval_request record, got ${approvalRequests.length}`);
  assert(approvalRequests.length >= 3, `Should have 3 approval_request records for 3 blocked actions, got ${approvalRequests.length}`);
  ...
  for (const ar of approvalRequests) {
    ...
    assert(ar.risk_level === "dangerous-global-mutation" || ar.risk_level === "hermes-scoped-mutation",
  ...
  for (const action of classifiedActions) {
    assert(action.status === "awaiting_approval",
      `Each unsafe action should have status "awaiting_approval", got "${action.status}" for action "${action.action_reference || action.action}"`);
    assert(action.approval_required === true,
      `Each unsafe action should have approval_required=true`);
  }

  // Verify proposal_records link actions to approval requests
  const proposalRecords = result.propose?.proposal_records || [];
  assert(proposalRecords.length === 3, `Should have 3 proposal_records, got ${proposalRecords.length}`);
  for (const pr of proposalRecords) {
    assertHasField(pr, "approval_request_memory_id", "proposal_record");
    assert(pr.link_status === "approval_requested",
      `Proposal record link_status should be "approval_requested", got "${pr.link_status}"`);
  }

  console.log(`  PASS: ${blockedActions.length} unsafe actions blocked with ${approvalRequests.length} approval request escalation records`);
}
...
async function testValOrch010_ReadOnlyActionsRemainUnblocked() {
  ...
  const result = await callTool("business_pm_loop", {
    ...
    proposed_actions: [
      { action: "research_market_trends", type: "research", description: "Read-only research on market trends" },
      { action: "query_vps_info", type: "read", description: "Read VPS status info" },
      { action: "list_recent_sessions", type: "list", description: "List recent Factory sessions" },
      { action: "generate_status_report", type: "report", description: "Generate business status report" },
      { action: "recall_prior_decisions", type: "recall", description: "Recall prior decision records from memory" },
    ],
  });

  const classifiedActions = result.propose?.actions || [];
  assert(classifiedActions.length === 5, `Should have 5 classified actions, got ${classifiedActions.length}`);
  ...
  for (const action of classifiedActions) {
    ...
    assert(action.risk_level === "read-only",
      `Action "${actionRef}" should be read-only, got ${action.risk_level}`);
    assert(action.approval_required === false,
  ...
  const blockedActions = result.propose?.blocked_actions || [];
  assert(blockedActions.length === 0,
  ...
  const riskPolicyCapabilityRequests = capabilityRequests.filter(
    (cr) => cr.source === "blocked_proposal" || cr.source === "online_portal_prerequisite");
  assert(riskPolicyCapabilityRequests.length === 0,
    `Should have no policy-driven capability requests for read-only actions, got ${riskPolicyCapabilityRequests.length}`);

  console.log("  PASS: Read-only actions remain non-blocked without approval/capability/task routing side effects");
}
...
async function testValOrch011_SessionBoundOnlineStepsRouteToLocalTaskQueue() {
  ...
  const result = await callTool("business_pm_loop", {
    ...
    correlation_id: correlationId,
    proposed_actions: [
      { action: "submit_taxnetusa_form", type: "submit portal form browser session", description: "Submit TaxNetUSA appraisal form via authenticated browser session", portal: "taxnetusa" },
      { action: "check_gmail_inbox", type: "portal browser", description: "Check Gmail inbox for order updates", portal: "gmail" },
      { action: "query_matrix_mls", type: "portal browser session", description: "Query Matrix MLS for comparable sales data", portal: "matrix_mls" },
    ],
  });

  // Verify online_step_classification exists
  const onlineSteps = result.propose?.online_step_classification || [];
  assert(onlineSteps.length === 3, `Should have 3 online step classifications, got ${onlineSteps.length}`);
  ...
  assert(browserLocalTasks.length >= 1,
  ...
  for (const task of browserLocalTasks) {
    ...
    assert(
      task.reason === "session_bound_online_step" || task.reason === "blocked_online_step",
  ...
  const sessionBoundActions = classifiedActions.filter(
    (a) => a.execution_classification === "session-bound" || a.execution_classification === "blocked");
  assert(sessionBoundActions.length >= 1,
  ...
  for (const action of sessionBoundActions) {
    assert(action.execution_classification !== "headless-safe",
      `Session-bound action "${action.action_reference || action.action}" should not be classified as headless-safe`);
    assert(action.portal_surface !== null,
      `Session-bound action should have a portal_surface`);
  }

  console.log(`  PASS: ${sessionBoundActions.length} session-bound online steps routed to ${browserLocalTasks.length} local browser tasks with required metadata`);
}
...
async function testValOrch012_DangerousMutationsBlockedWithApprovalRouting() {
  ...
  const result = await callTool("business_pm_loop", {
    ...
    correlation_id: correlationId,
    proposed_actions: [
      { action: "full_vps_restart", type: "restart full VPS", description: "Full VPS restart — dangerous global mutation" },
      { action: "stop_unrelated_project", type: "stop project nginx-proxy", description: "Stop production nginx proxy — dangerous global" },
      { action: "create_vps_snapshot", type: "snapshot", description: "Create VPS snapshot — dangerous global" },
    ],
  });
  ...
  for (const action of classifiedActions) {
    ...
    assert(action.risk_level === "dangerous-global-mutation",
  ...
  const approvalRequests = result.propose?.approval_requests || [];
  assert(approvalRequests.length === 3,
  ...
  for (const ar of approvalRequests) {
    ...
    assertHasField(ar, "action_reference", "approval_request");
    assertHasField(ar, "risk_level", "approval_request");
    assert(ar.risk_level === "dangerous-global-mutation",
  ...
  const directlyExecutable = classifiedActions.filter((a) => a.status === "ready");
  assert(directlyExecutable.length === 0,
  ...
  assert(dangerousRecords.length === 3,
  ...
  console.log("  PASS: Dangerous mutations are blocked, never directly executable, and routed to approval workflow");
}
...
async function testValOrch015_OnlineBlockerCapabilityRequestsDeduplicatedByBlockerKey() {
  ...
  const capabilityRequests = result.propose?.capability_requests || [];
  assert(capabilityRequests.length >= 2,
  ...
  const taxnetReused = taxnetRequests.filter((cr) => cr.status === "reused_existing");
  assert(taxnetReused.length >= taxnetRequests.length - 1,
