...
async function testValAudit004_StatusReportGovernanceSections() {
  ...
  for (const section of GOVERNANCE_REQUIRED_SECTIONS) {
    ...
    if (section === "next_steps") {
      assert(
        ...
        `business_status_report: next_steps must be non-empty array, got ${JSON.stringify(
          report.next_steps,
        )}`,
      );
    }
  ...
  const loopResult = await callTool("business_pm_loop", {
    ...
    observations: [
      {
        type: "test_signal",
        summary: "Governance reporting validation signal",
        source: "test",
  ...
  assert(
    Array.isArray(inlineSR.next_steps) && inlineSR.next_steps.length > 0,
    `PM loop inline status_report: next_steps must be non-empty array, got ${JSON.stringify(
      inlineSR.next_steps,
    )}`,
  );
  ...
  if (inlineSR.risks.length > 0) {
    ...
    assert(
      typeof firstRisk === "object" && firstRisk !== null,
      "Each risk must be an object",
    );
    assertHasField(firstRisk, "risk_level", "risk entry");
  }
  console.log(
    "  => All governance sections present in PM loop inline status_report",
  );

  console.log(
    "  PASS: VAL-AUDIT-004 Governance-required sections present in both status report surfaces",
  );
}
...
async function testValAudit005_StatusReportContextConsistency() {
  ...
  const loopResult = await callTool("business_pm_loop", {
    ...
    observations: [
      {
        type: "context_test",
        summary: "Context consistency signal",
        source: "test",
        timestamp: new Date().toISOString(),
        confidence: "high",
      },
  ...
  assert(
    typeof inlineSR.generated_at === "string" &&
      inlineSR.generated_at.length > 0,
  ...
  assert(
    inlineSR.current_focus === objective,
    `current_focus should match objective: expected "${objective}", got "${inlineSR.current_focus}"`,
  );

  console.log(
    "  => PM loop inline status_report: correlation_id, generated_at, current_focus all consistent",
  );
  ...
  assert(
    typeof report.current_focus === "string" && report.current_focus.length > 0,
    "current_focus must be a non-empty string",
  );
  assert(
    report.current_focus === focus,
    `current_focus should match focus param: expected "${focus}", got "${report.current_focus}"`,
  );

  console.log(
    "  => business_status_report: correlation_id, generated_at, current_focus all consistent",
  );

  console.log(
    "  PASS: VAL-AUDIT-005 Status report context remains consistent with active cycle",
  );
}
...
async function testValAudit009_KpiSloCounters() {
  ...
  const unknownSessionId = `missing-session-${Date.now()}-${Math.random()
    .toString(36)
    .slice(2, 10)}`;
  ...
  for (const field of syncCounterFields) {
    assert(
      typeof syncSummary[field] === "number",
      `sync summary.${field} must be a number, got ${typeof syncSummary[
        field
      ]}: ${syncSummary[field]}`,
    );
  }
  console.log(
    `  => factory_sync_sessions summary: ${JSON.stringify(syncSummary)}`,
  );

  // Verify completion gate config is present
  assertHasField(syncResult, "completion_gate", "factory_sync_sessions");
  assert(
    typeof syncResult.completion_gate.min_confidence === "number" ||
      syncResult.completion_gate.min_confidence === null,
    `completion_gate.min_confidence must be a number (or null), got ${typeof syncResult
      .completion_gate.min_confidence}: ${
      syncResult.completion_gate.min_confidence
    }`,
  );
  assertHasField(
    syncResult.completion_gate,
    "require_citations",
    "completion_gate",
  );
    ...
    proposed_actions: [
      {
        ...
        description: "Dangerous action to generate blocked counters",
      },
    ],
  });
  ...
  for (const field of requiredKpiFields) {
    assert(
      typeof kpi[field] === "number",
      `kpi_counters.${field} must be a number, got ${typeof kpi[field]}: ${
        kpi[field]
      }`,
    );
  }
  ...
  assert(
    Array.isArray(meta.pending_retries),
    `metadata.pending_retries must be an array`,
  );
  assert(
    Array.isArray(hb.blocked_capabilities),
    `heartbeat.blocked_capabilities must be an array, got ${typeof hb.blocked_capabilities}`,
  );

  console.log(
    `  => PM loop metadata: pending_retries=${meta.pending_retries.length}, heartbeat blocked_capabilities=${hb.blocked_capabilities.length}`,
  );
  ...
  for (const field of srKpiFields) {
    assert(
      typeof srKpi[field] === "number",
      `business_status_report.kpi_counters.${field} must be a number, got ${typeof srKpi[
        field
      ]}: ${srKpi[field]}`,
    );
  }
  ...
  console.log(
    `  => business_status_report KPI counters: ${JSON.stringify(srKpi)}`,
  );

  console.log(
    "  PASS: VAL-AUDIT-009 KPI/SLO counters are machine-readable across all key surfaces",
  );
}
...
async function testValAudit010_MutationAuditProvenance() {
    ...
    proposed_actions: [
      {
        ...
        type: "restart hermes",
        description: "Hermes service restart requiring validation+approval",
      },
    ],
    learnings: [
      {
  ...
  const decisionRecords = Array.isArray(learnSection.decision_records)
    ? learnSection.decision_records
    : [];
  assert(
  ...
  console.log(
    `  => Memory decisions matching "mutation_audit": ${decisionRows.length}`,
  );
  ...
  const approvalRequests = Array.isArray(proposeSection.approval_requests)
    ? proposeSection.approval_requests
    : [];
  assert(
  ...
  assert(
    firstApproval.risk_level === "hermes-scoped-mutation",
  ...
  const matching = recalledRows.filter(
    (r) =>
      typeof r.content === "string" &&
      r.content.includes("vps_deploy") &&
      r.category === "decision",
  );
  assert(
    matching.length > 0,
  ...
  assert(
    meta.tool === "vps_deploy",
    `Expected tool=vps_deploy, got ${meta.tool}`,
  );
  assert(
    meta.risk_level === "hermes-scoped-mutation",
    `Expected risk_level=hermes-scoped-mutation, got ${meta.risk_level}`,
  );
  assert(
    meta.validation_id === "VAL-AUDIT-010-TEST",
    `Expected validation_id=VAL-AUDIT-010-TEST, got ${meta.validation_id}`,
  );
  assert(
    meta.approver === "test-validator",
    `Expected approver=test-validator, got ${meta.approver}`,
  );
  assert(
    meta.deployed_commit === "current-build-commit",
    `Expected deployed_commit=current-build-commit, got ${meta.deployed_commit}`,
  );

  // Verify timestamp is present
  assert(
    typeof meta.at === "string" && meta.at.length > 0,
    "mutation_audit must include 'at' timestamp",
  );
  assert(
    meta.at === auditTimestamp,
    `Timestamp should match: expected ${auditTimestamp}, got ${meta.at}`,
  );

  // Verify decision_class is mutation_audit
  assert(
    meta.decision_class === "mutation_audit",
    `Expected decision_class=mutation_audit, got ${meta.decision_class}`,
  );

  console.log(
    `  => All provenance fields verified: tool=${meta.tool}, risk_level=${meta.risk_level}, validation_id=${meta.validation_id}, approver=${meta.approver}, deployed_commit=${meta.deployed_commit}, at=${meta.at}`,
  );

  console.log(
    "  PASS: VAL-AUDIT-010 Approved mutating actions persist mutation_audit decision records with provenance",
  );
}
...
async function main() {
  ...
  const tests = [
    {
      name: "VAL-AUDIT-004",
      fn: testValAudit004_StatusReportGovernanceSections,
    },
    {
      name: "VAL-AUDIT-005",
      fn: testValAudit005_StatusReportContextConsistency,
    },
  ...
  console.log(`\n========================================`);
  console.log(
    `Governance Reporting & KPI: ${passed} passed, ${failed} failed, ${tests.length} total`,
  );
