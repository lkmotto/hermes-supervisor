...
async function testValAudit001_CorrelationIdPreservation() {
  ...
  assert(
    cycleResult.correlation_id === cid,
  ...
  assert(
    artifactsWithCorrelation > 0,
    `No artifacts found carrying correlation_id ${cid}; ${artifactsWithCorrelation}/${artifacts.length}`,
  );
  console.log(
    `  => ${artifactsWithCorrelation}/${artifacts.length} artifacts carry correlation_id`,
  );
  ...
  const matchingRows = memRows.filter(
    (r) => typeof r.content === "string" && r.content.includes(cid),
  );
  assert(
    matchingRows.length > 0,
    `No memory records found for correlation ${cid}`,
  );
  ...
  assert(
    memWithCorrelation > 0,
    `No memory records carry correlation_id in metadata; ${memWithCorrelation}/${matchingRows.length}`,
  );
  console.log(
    `  => ${memWithCorrelation}/${matchingRows.length} memory records carry correlation_id in metadata`,
  );

  // 5. Verify emitted_sections carry run_id linkage
  const emittedSections = Array.isArray(cycleResult.emitted_sections)
    ? cycleResult.emitted_sections
    : [];
  ...
  for (const section of emittedSections) {
    if (section.event_id !== null && section.event_id !== undefined)
      sectionsWithEventId += 1;
    if (section.artifact_id !== null && section.artifact_id !== undefined)
      sectionsWithArtifactId += 1;
  }
  console.log(
    `  => Sections: ${emittedSections.length} total, ${sectionsWithEventId} with event_id, ${sectionsWithArtifactId} with artifact_id`,
  );

  console.log(
    `  PASS: VAL-AUDIT-001 correlation_id=${cid} propagated across run(${runId}), ${artifactsWithCorrelation} artifacts, ${memWithCorrelation} memory records`,
  );
}
...
async function testValAudit002_SectionMaterialization() {
  ...
  for (const req of requiredSections) {
    assert(
      foundSections.has(req),
      `Required section "${req}" missing from emitted_sections. Found: ${[
        ...foundSections,
      ]
        .sort()
        .join(", ")}`,
  ...
  assert(
    foundSections.has("business_pm_output"),
    `business_pm_output section missing. Found: ${[...foundSections]
      .sort()
      .join(", ")}`,
  );
  ...
  for (const section of emittedSections) {
    if (section.section === "business_pm_output") {
      ...
      const hasBoth =
        section.event_id !== null &&
        section.event_id !== undefined &&
        section.artifact_id !== null &&
        section.artifact_id !== undefined;
      if (hasBoth) sectionsWithBoth += 1;
    }
  }
  console.log(
    `  => ${sectionsWithBoth}/${requiredSections.length} canonical sections have both event_id and artifact_id`,
  );
  ...
  console.log(
    `  => Fleet run status: ${run.status}, events: ${
      (run.event_ids || []).length
    }, artifacts: ${(run.artifact_ids || []).length}`,
  );

  console.log(
    `  PASS: VAL-AUDIT-002 ${emittedSections.length} sections materially recorded with section-level write IDs`,
  );
}
...
async function testValAudit003_TypedTraceabilityMetadata() {
  ...
  for (const row of matchingRows) {
    ...
    if (missingFields.length === 0) {
      ...
      console.log(
        `  WARN: memory record ${row.id} (category: ${
          row.category
  ...
  console.log(
    `  => ${recordsWithAllFields}/${matchingRows.length} memory records carry source, timestamp, confidence, correlation_id`,
  );

  // Verify cycle-level validation record specifically
  const validationRows = matchingRows.filter(
    (r) => r.category === "validation",
  );
  if (validationRows.length > 0) {
    ...
    console.log(
      `  => Validation record carries run_id=${runId}, matching cycle`,
    );
  }

  console.log(
    `  PASS: VAL-AUDIT-003 Typed traceability metadata present across ${recordsWithAllFields} records`,
  );
}
...
async function testValAudit006_DegradedOutcomesExplicit() {
  ...
  assert(
    status === "degraded" ||
      status === "error" ||
      degradedResult._raw?.includes("degraded"),
  ...
  if (degradedResult.correlation_id) {
    assert(
      degradedResult.correlation_id === cid2,
      `correlation_id should match: expected ${cid2}, got ${degradedResult.correlation_id}`,
    );
    console.log(
      `  => Degraded response carries correlation_id=${degradedResult.correlation_id}`,
    );
  }

  // Verify retry telemetry exists
  if (degradedResult.pending_retries) {
    const retries = Array.isArray(degradedResult.pending_retries)
      ? degradedResult.pending_retries
      : [];
    console.log(`  => Degraded response has ${retries.length} pending retries`);
    if (retries.length > 0) {
      for (const retry of retries) {
        assert(
          retry.correlation_id === cid2,
          `Retry entry correlation_id should match: expected ${cid2}, got ${retry.correlation_id}`,
        );
        assert(
          typeof retry.operation === "string" && retry.operation.length > 0,
          "Retry entry should have operation field",
        );
        assert(
          typeof retry.error === "string" && retry.error.length > 0,
          "Retry entry should have error description",
        );
      }
      console.log(
        `  => All retry entries carry correlation_id and operation details`,
      );
    }
  }
  ...
  assert(
    ksStatus === "degraded" ||
      ksStatus === "error" ||
      (ksFailResult._raw && ksFailResult._raw.includes("degraded")),
  ...
  assert(
    ksRecordId === null || ksRecordId === undefined,
  ...
  const ksRetries = Array.isArray(ksFailResult.pending_knowledge_retries)
    ? ksFailResult.pending_knowledge_retries
    : [];
  assert(
    ksRetries.length > 0,
  ...
  for (const retry of ksRetries) {
    assert(
      retry.correlation_id === cid3,
      `Knowledge retry correlation_id should match: expected ${cid3}, got ${retry.correlation_id}`,
    );
  }
  console.log(
    `  => Knowledge-store failure: status=${ksStatus}, knowledge_record_id=${ksRecordId}, pending_knowledge_retries=${ksRetries.length}`,
  );

  console.log(
    `  PASS: VAL-AUDIT-006 Degraded outcomes are explicit with reason, correlation, and retry telemetry`,
  );
}
...
async function testValAudit007_SecretRedaction() {
  ...
  console.log(
    "  => Seeded memory record with benign audit metadata and probe correlation",
  );
  ...
  console.log(
    "  => Benign audit metadata (validation_id, run_id, goal, correlation_id) preserved",
  );
  ...
  console.log(
    "  => Redaction sweep regression test independently validates zero residual findings",
  );

  console.log(
    `  PASS: VAL-AUDIT-007 Secret redaction protects content while preserving benign audit metadata`,
  );
}
...
async function testValAudit008_CrossSurfaceSecretSafety() {
  ...
  console.log(
    "  => Cross-surface auditability test independently validates VAL-CROSS-005 (10 surfaces)",
  );

  console.log(
    `  PASS: VAL-AUDIT-008 Secret safety holds across all ${surfaces.length} operational surfaces`,
  );
}
...
async function testValAudit011_KnowledgeStoreFailureExplicit() {
  ...
  const ksRetries = Array.isArray(cycleResult.pending_knowledge_retries)
  ...
  const cycleRetry = ksRetries.find(
    (r) => r.operation === "cycle_knowledge_write",
  );
  assert(
    cycleRetry !== undefined,
    `Expected cycle_knowledge_write retry entry, got operations: ${ksRetries
      .map((r) => r.operation)
      .join(", ")}`,
  );
  ...
  assert(
    cycleRetry.correlation_id === cid,
    `Retry correlation_id should match: expected ${cid}, got ${cycleRetry.correlation_id}`,
  );
  assert(
    typeof cycleRetry.error === "string" && cycleRetry.error.length > 0,
    "Retry entry should have error description",
  );
  assert(
    cycleRetry.status === "pending_retry",
    `Retry status should be pending_retry, got: ${cycleRetry.status}`,
  );
  assert(
    cycleRetry.failure_surface === "knowledge_store",
    `Retry failure_surface should be knowledge_store, got: ${cycleRetry.failure_surface}`,
  );

  console.log(
    `  => Retry entry: operation=${cycleRetry.operation}, status=${cycleRetry.status}, failure_surface=${cycleRetry.failure_surface}`,
  );

  // 6. Verify errors array contains knowledge_store entry
  const errors = Array.isArray(cycleResult.errors) ? cycleResult.errors : [];
  const ksError = errors.find(
    (e) => typeof e === "string" && e.includes("knowledge_store"),
  );
  assert(
    ksError !== undefined,
  ...
  assert(
    cycleResult.run_id !== null && cycleResult.run_id !== undefined,
  ...
  if (cycleResult.heartbeat) {
    ...
    const blocked = Array.isArray(hb.blocked_capabilities)
      ? hb.blocked_capabilities
      : [];
    assert(
      blocked.includes("knowledge_store_pending_retry"),
  ...
  console.log(
    `  PASS: VAL-AUDIT-011 Knowledge-store write failure produces degraded state, null knowledge_record_id, and correlated retry entry`,
  );
}
...
async function main() {
  ...
  const tests = [
    ...
    {
      name: "VAL-AUDIT-011",
      fn: testValAudit011_KnowledgeStoreFailureExplicit,
    },
  ];
  ...
  console.log(`\n========================================`);
  console.log(
    `Audit Assertion Verification: ${passed} passed, ${failed} failed, ${tests.length} total`,
  );
