...
async function testValOnline001() {
  ...
  const text = await callToolRaw("research", {
    query:
      "Give one short factual sentence about residential appraisal workflow automation trends in 2026.",
  });
  assert(text.length > 40, "research response should be substantive");
  assert(
    !/mock|stub|placeholder/i.test(text),
    "research response should not look mocked",
  );
  console.log(
    "  PASS: research tool is available and returns live response text",
  );
}
...
async function testValOnline004SeededAwareness() {
  ...
  const seeded =
    result.perceive?.workflow_summary?.seeded_asset_awareness || {};
  const sourcePaths = seeded.source_paths || [];
  assert(
    sourcePaths.some((p) => String(p).includes("/root/missions/neon-wf1")),
    "seeded source paths should include neon-wf1",
  );
  assert(
    sourcePaths.some((p) => String(p).includes("/opt/motto-skills")),
    "seeded source paths should include motto-skills",
  );
  assert(
    (seeded.wf1_steps || []).length > 0,
    "seeded wf1 steps should be extracted",
  );
  assert(
    (seeded.order_intake_fields || []).length > 0,
    "order-intake fields should be extracted",
  );
  console.log(
    "  PASS: workflow awareness seeded from WF1 and order-intake assets",
  );
}
...
async function testValOnline002003013() {
    ...
    proposed_actions: [
      {
        ...
        description:
          "TaxNetUSA lookup is blocked: login required and session unavailable",
        missing_prerequisites: [
          "taxnetusa_authenticated_session",
          "taxnetusa_mfa_access",
        ],
      },
    ],
  });
  ...
  assert(
    ...
    "missing portal prerequisites should create capability requests",
  );
  const onlineReq = capabilityRequests.find((req) =>
    String(req.source || "").includes("online_portal_prerequisite"),
  );
  assert(
    onlineReq,
    "should include online portal prerequisite capability request",
  );
  assert(
    String(onlineReq.portal_surface || "") === "taxnetusa",
    "portal surface should be taxnetusa",
  );

  const localTasks = result.propose?.local_tasks || [];
  assert(
    localTasks.length > 0,
    "session-bound workflow should queue a local task",
  );
  ...
  for (const need of authNeeds) {
    assert(
      typeof need.type === "string" && need.type.length > 0,
      "auth need must include type",
    );
    assert(
      typeof need.handle === "string" && need.handle.length > 0,
      "auth need must include handle",
    );
    assert(
      !/[=:\s]/.test(need.handle),
      "auth handle should be normalized and non-secret-like",
    );
  }
  console.log(
    "  PASS: blocked portal step became capability + local task with sanitized auth/session requirements",
  );
}
...
async function testValOnline005Persistence() {
  ...
  const capabilityRows = await callTool("memory_recall", {
    category: "capability_gap",
    query: cid,
    limit: 20,
  });
  const workflowRows = await callTool("memory_recall", {
    category: "workflow",
    query: cid,
    limit: 20,
  });
  const learningRows = await callTool("memory_recall", {
    category: "learning",
    query: cid,
    limit: 20,
  });
  ...
  const total =
    (Array.isArray(capabilityRows) ? capabilityRows.length : 0) +
  ...
  console.log(
    "  PASS: online observations were persisted with typed categories",
  );
}
...
async function testValOnline006007() {
  ...
  const result = await callTool("business_pm_loop", {
    ...
    proposed_actions: [
      {
        action: "research_appraisal_regulations",
        type: "research",
        description: "Headless-safe research step",
      },
      {
        action: "open_gmail_order_thread",
        type: "gmail browser session review",
        description: "Session-bound Gmail review step",
      },
      {
  ...
  const classSet = new Set(
    classes.map((entry) => entry.execution_classification),
  );
  assert(
    classSet.has("headless-safe"),
    "should classify at least one step as headless-safe",
  );
  assert(
    classSet.has("session-bound"),
    "should classify at least one step as session-bound",
  );
  assert(
    classSet.has("blocked"),
    "should classify at least one step as blocked",
  );

  const continuation =
    result.status_report?.online_learning?.continue_safe_learning_when_blocked;
  assert(
    continuation === true,
    "safe online learning should continue when portal steps are blocked",
  );
  console.log(
    "  PASS: mixed steps are classified and blocked portal work does not halt safe learning",
  );
}
...
async function testValOnline014DedupCorrelation() {
  ...
  assert(
    firstReq,
    "first run should create online portal prerequisite request",
  );
  ...
  assert(
    secondReq,
    "second run should include online portal prerequisite record",
  );

  const reused =
    String(secondReq.source || "").includes("reused") ||
    (firstReq.request_id &&
      secondReq.request_id &&
      firstReq.request_id === secondReq.request_id);
  assert(
    reused,
    "repeated blocker should reuse/reference existing capability request",
  );
  assert(
    firstReq.blocker_key &&
      secondReq.blocker_key &&
      firstReq.blocker_key === secondReq.blocker_key,
    "repeated blocker should carry the same blocker correlation key",
  );
  console.log(
    "  PASS: repeated blocker reused correlated request instead of creating uncorrelated spam",
  );
}
