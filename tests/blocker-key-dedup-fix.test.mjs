...
async function testCrossCycleBlockerKeyDedup() {
  ...
  assert(firstReq, "Cycle 1: should create an online portal prerequisite request");
  assert(firstReq.status === "pending",
    `Cycle 1: request should be "pending", got "${firstReq.status}"`);
  assert(typeof firstReq.blocker_key === "string" && firstReq.blocker_key.length > 0,
    "Cycle 1: request should have non-empty blocker_key");
  assert(typeof firstReq.request_id === "string" && firstReq.request_id.length > 0,
  ...
  console.log(`  Cycle 1 persisted gap memory_id: ${matchGap.memory_id || matchGap.id}`);
  ...
  assert(secondReq, "Cycle 2: should include an online portal prerequisite record");

  const isReused = String(secondReq.source || "").includes("reused")
...
async function testWithinCycleBlockerKeyDedup() {
  ...
  assert(reused[0].blocker_key === pending[0].blocker_key,
    "Within-cycle: reused and pending should share same blocker_key");
  assert(reused[0].request_id === pending[0].request_id,
    "Within-cycle: reused should share same request_id");

  console.log(`  PASS: Within-cycle dedup: 1 pending + 1 reused sharing blocker_key and request_id`);
}
...
async function testBlockerKeyWithUnderscores() {
  ...
  assert(secondReq.blocker_key === firstKey,
  ...
  console.log(`  PASS: Underscore-rich blocker_key correctly matched across cycles`);
}
